...
HEADERS = base64simple.h
LIBRARIES = libbase64simple.so

.PHONY: all install uninstall clean

all: $(LIBRARIES)
	
libbase64simple.so: $(OBJ)/libbase64simple.o | $(LIB)
	$(CC) -shared -o $(LIB)/$@ $^
...
install:
	...
	if [ -d /etc/ld.so.conf.d ]; then \
		echo "$(DESTDIR)$(libdir)" > /etc/ld.so.conf.d/libbase64simple.conf ; \
	fi
	ldconfig
